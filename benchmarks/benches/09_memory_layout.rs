...
fn make_array(size: usize) -> Vec<f64> {
...
impl<T: Default + Clone, const N: usize> SpscQueue<T, N> {
    ...
    pub fn try_push(&self, item: T) -> bool {
        ...
        unsafe {
            *self.buffer[(head as usize) & (N - 1)].get() = item;
        }
...
fn bench_false_sharing(c: &mut Criterion) {
    let bad = FalseSharingBad {
        counter_a: AtomicU64::new(0),
        counter_b: AtomicU64::new(0),
    };
    let good = FalseSharingGood {
        counter_a: CachePaddedCounter {
            value: AtomicU64::new(0),
            _pad: [0; 56],
        },
        counter_b: CachePaddedCounter {
            value: AtomicU64::new(0),
            _pad: [0; 56],
        },
    };
    let mut group = c.benchmark_group("false_sharing");
    group.bench_function("bad_layout_single_thread", |b| {
        b.iter(|| {
            bad.counter_a.fetch_add(1, Ordering::Relaxed);
            black_box(bad.counter_b.load(Ordering::Relaxed));
        })
    });
    group.bench_function("padded_layout_single_thread", |b| {
        b.iter(|| {
            good.counter_a.value.fetch_add(1, Ordering::Relaxed);
            black_box(good.counter_b.value.load(Ordering::Relaxed));
        })
    });
    group.finish();
}
...
fn bench_memory_access_patterns(c: &mut Criterion) {
    ...
    group.bench_function("l3_sequential", |b| {
        b.iter(|| black_box(sequential_sum(black_box(&l3_data))))
    });
    group.bench_function("l3_stride_64", |b| {
        b.iter(|| black_box(strided_sum(black_box(&l3_data), 64)))
    });
    group.finish();
}
