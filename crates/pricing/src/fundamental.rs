pub fn calculate_wacc(equity: f64, debt: f64, cost_of_equity: f64, cost_of_debt: f64, tax_rate: f64) -> f64 {
    let v = equity + debt;
    if v == 0.0 { return 0.0; }
...
pub fn bquant_multifactor_score(pb: f64, roe: f64, ocf_to_assets: f64, earnings_yield: f64, momentum: f64) -> f64 {
...
pub fn universe_ranker_quartiles(scores: &[(String, f64)]) -> Vec<(String, &'static str)> {
    ...
    sorted.into_iter().enumerate().map(|(i, (symbol, _))| {
        let label = if i < qsize { "Long" }
        else if i >= n - qsize { "Short" }
        else { "Neutral" };
        (symbol, label)
    }).collect()
}
